<template>
    <div @click="$emit('navigate', plant.id)"
        ...
        <div class="relative h-48 overflow-hidden rounded-t-lg">
            <img :src="plant.image_url || noImage" :alt="plant.common_name"
                :class="[
            ...
            <div class="absolute top-2 right-2 flex flex-wrap gap-2">
                <span class="text-sm bg-green-100 text-green-800 px-2 py-1 rounded">
                    {{ plant.family || 'Nom inconnu' }}
                </span>
                <span class="text-sm bg-blue-100 text-blue-800 px-2 py-1 rounded">
                    {{ plant.genus || 'Nom inconnu' }}
                </span>
            </div>
        </div>
        <!-- Plant name section -->
        <div class="m-2 p-4">
            ...
            <p class="text-slate-400 italic">{{ plant.scientific_name || 'Nom inconnu' }}</p>
        </div>
    </div>
</template>
